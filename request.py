...
phone_link = link + "?query=" + phone_number
...
if user_id != -1:
    ...
    update_request = requests.patch(link, headers=req_headers, json=up_json)
...
# if the user does not exists - creating
    ...
    create_request = requests.post(link, headers=req_headers, json=cr_json)
